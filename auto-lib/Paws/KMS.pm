...
which data is encrypted, such as when you manually rotate a CMK or
change the CMK that protects a ciphertext. You can also use it to
reencrypt ciphertext under the same CMK, such as to change the
...
This operation requires an asymmetric CMK with a C<KeyUsage> value of
C<SIGN_VERIFY>.

Verification confirms that an authorized user signed the message with
the specified key and signing algorithm, and the message hasn't changed
since it was signed. A digital signature is generated by using the
private key in an asymmetric CMK. The signature is verified by using
the public key in the same asymmetric CMK. For information about
...
Specify the same asymmetric CMK that was used by the C<Sign> operation
...
The advantage of using the C<Verify> operation is that it is performed
...
The result of the C<Verify> operation, which is represented by its HTTP
...
see the C<SignatureValid> field in the response.

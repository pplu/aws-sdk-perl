package Paws::EC2::RegisterInstanceTagAttributeRequest;
  use Moose;
...
Paws::EC2::RegisterInstanceTagAttributeRequest
...
As an example, if Att1 is expected to be a Paws::EC2::RegisterInstanceTagAttributeRequest object:

  $service_obj->Method(Att1 => { IncludeAllTagsOfInstance => $value, ..., InstanceTagKeys => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::EC2::RegisterInstanceTagAttributeRequest object:
...
Specify C<true> to register all tag keys.

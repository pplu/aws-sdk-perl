...
package Paws::CodePipeline::ActionTypeArtifactDetails;
  use Moose;
...
Paws::CodePipeline::ActionTypeArtifactDetails
...
As an example, if Att1 is expected to be a Paws::CodePipeline::ActionTypeArtifactDetails object:

  $service_obj->Method(Att1 => { MaximumCount => $value, ..., MinimumCount => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CodePipeline::ActionTypeArtifactDetails object:
...
artifacts for an action type with a category of C<source>.

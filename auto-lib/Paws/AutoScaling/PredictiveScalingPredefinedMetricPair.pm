...
package Paws::AutoScaling::PredictiveScalingPredefinedMetricPair;
  use Moose;
...
Paws::AutoScaling::PredictiveScalingPredefinedMetricPair
...
As an example, if Att1 is expected to be a Paws::AutoScaling::PredictiveScalingPredefinedMetricPair object:

  $service_obj->Method(Att1 => { PredefinedMetricType => $value, ..., ResourceLabel => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::AutoScaling::PredictiveScalingPredefinedMetricPair object:
...
metric. For example, if the metric type is C<ASGCPUUtilization>, the
Auto Scaling group's total CPU metric is used as the load metric, and

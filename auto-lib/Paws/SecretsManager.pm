...
This guide provides descriptions of the AWS Secrets Manager API. For
more information about using this service, see the AWS Secrets Manager
User Guide
(http://docs.aws.amazon.com/http:/docs.aws.amazon.com/secretsmanager/latest/userguide/introduction.html).

B<API Version>

This version of the AWS Secrets Manager API Reference documents the AWS
...
to AWS Secrets Manager. However, you also can use the AWS Secrets
Manager HTTP Query API to make direct calls to the AWS Secrets Manager
web service. To learn more about the AWS Secrets Manager HTTP Query
API, see Making Query Requests
(http://docs.aws.amazon.com/secretsmanager/latest/userguide/orgs_query-requests.html)
in the I<AWS Secrets Manager User Guide>.

AWS Secrets Manager supports GET and POST requests for all actions.
That is, the API doesn't require you to use GET for some actions and
POST for others. However, GET requests are subject to the limitation
size of a URL. Therefore, for operations that require larger sizes, use
a POST request.
...
strongly recommend that you don't create an access key for your root
...
create an access key for an IAM user account that has the permissions
required for the task at hand. As another option, use AWS Security
Token Service to generate temporary security credentials, and use those
credentials to sign requests.

To sign requests, you must use Signature Version 4
(http://docs.aws.amazon.com/general/latest/gr/signature-version-4.html).
If you have an existing application that uses Signature Version 2, you
must update it to use Signature Version 4.
...
The JSON that AWS Secrets Manager returns as a response to your
requests is a single long string without line breaks or white space
formatting. Both line breaks and white space are included in the
examples in this guide to improve readability. When example input
parameters would also result in long strings that extend beyond the
...
can determine which requests were successfully made to AWS Secrets
Manager, who made the request, when it was made, and so on. For more
about AWS Secrets Manager and its support for AWS CloudTrail, see
Logging AWS Secrets Manager Events with AWS CloudTrail
(http://docs.aws.amazon.com/secretsmanager/latest/userguide/orgs_cloudtrail-integration.html)
...
AWS Secrets Manager uses the account's default AWS managed customer
master key (CMK) with the alias C<aws/secretsmanager>. If this key
doesn't already exist in your account then AWS Secrets Manager creates
it for you automatically. All users in the same AWS account
automatically have access to use the default CMK. Note that if an AWS
...
value then you must create and use a custom KMS CMK because you can't
access the default CMK for the account using credentials from a
...
kms:GenerateDataKey - needed only if you use a customer-created KMS key
...
you don't provide a recovery window value, the operation defaults to 30
...
version. That marks the version as deprecated and allows AWS Secrets
...
secret, use ListSecretVersionIds. The encrypted fields C<SecretString>
and C<SecretBinary> are not included in the output. To get that
...
The AWS Secrets Manager console uses only the C<SecretString> field. To
add binary data to a secret with the C<SecretBinary> field you must use
...
The AWS Secrets Manager console uses only the C<SecretString> parameter

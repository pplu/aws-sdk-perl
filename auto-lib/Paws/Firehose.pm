...
C<RedshiftDestinationConfiguration> or C<S3DestinationConfiguration>

As part of C<S3DestinationConfiguration>, optional values
C<BufferingHints>, C<EncryptionConfiguration>, and C<CompressionFormat>
can be provided. By default, if no C<BufferingHints> value is provided,
Amazon Kinesis Firehose buffers data up to 5 MB or for 5 minutes,
whichever condition is satisfied first. Note that C<BufferingHints> is
a hint, so there are some cases where the service cannot adhere to
these conditions strictly; for example, record boundaries are such that
...
A few notes about C<RedshiftDestinationConfiguration>:

=over

=item * An Amazon Redshift destination requires an S3 bucket as
intermediate location, as Amazon Kinesis Firehose first delivers data
to S3 and then uses C<COPY> syntax to load data into an Amazon Redshift
table. This is specified in the
C<RedshiftDestinationConfiguration.S3Configuration> parameter element.

=item * The compression formats C<SNAPPY> or C<ZIP> cannot be specified
in C<RedshiftDestinationConfiguration.S3Configuration> because the
...
is used exclusively for Amazon Kinesis Firehose purposes, and that the
...
Amazon S3 Bucket Access in the I<Amazon Kinesis Firehose Developer
Guide>.
...
consumer application(s) to parse individual data items when reading the
data from the destination.

Amazon Kinesis Firehose does not maintain data record ordering. If the
destination data needs to be re-ordered by the consumer application,
the producer should include some form of sequence number in each data
record.

The PutRecord operation returns a C<RecordId>, which is a unique string
assigned to each record. Producer applications can use this ID for
purposes such as auditability and investigation.

If the PutRecord operation throws a C<ServiceUnavailableException>,
...
C<FailedPutCount>, and an array of responses, C<RequestResponses>. The
C<FailedPutCount> value is a count of records that failed. Each entry
in the C<RequestResponses> array gives additional information of the
processed record. Each entry in C<RequestResponses> directly correlates
with a record in the request array using the same ordering, from the
top to the bottom of the request and response. C<RequestResponses>
always includes the same number of records as the request array.
C<RequestResponses> both successfully and unsuccessfully processed
records. Amazon Kinesis Firehose attempts to process all records in
...
record includes C<ErrorCode> and C<ErrorMessage> values. C<ErrorCode>
...
If C<FailedPutCount> is greater than 0 (zero), retry the request. A
...
C<ServiceUnavailableException>, back off and retry. If the exception
...
Amazon Kinesis Firehose uses the C<CurrentDeliveryStreamVersionId> to
...
existing configuration matches the C<VersionId>. After the update is
applied successfully, the C<VersionId> is updated, which can be

...
Paws::SageMaker::TransformDataSource
...
As an example, if Att1 is expected to be a Paws::SageMaker::TransformDataSource object:

  $service_obj->Method(Att1 => { S3DataSource => $value, ..., S3DataSource => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SageMaker::TransformDataSource object:

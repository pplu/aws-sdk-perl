...
package Paws::IAM::EnableMFADevice;
  ...
  has AuthenticationCode1 => (is => 'ro', isa => Str, required => 1, predicate => 1);
  has AuthenticationCode2 => (is => 'ro', isa => Str, required => 1, predicate => 1);
  has SerialNumber => (is => 'ro', isa => Str, required => 1, predicate => 1);
  ...
  'IsRequired' => {
                    'UserName' => 1,
                    'SerialNumber' => 1,
                    'AuthenticationCode2' => 1,
                    'AuthenticationCode1' => 1
                  }
}
;
    return $Params_map;

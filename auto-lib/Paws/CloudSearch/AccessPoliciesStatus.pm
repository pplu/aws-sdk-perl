...
As an example, if Att1 is expected to be a Paws::CloudSearch::AccessPoliciesStatus object:

  $service_obj->Method(Att1 => { Options => $value, ..., Status => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::CloudSearch::AccessPoliciesStatus object:

...
C<String> types. The pattern must match the entire parameter value
provided.
...
allowed pattern of C<[A-Za-z0-9]+> displays the following error message
when the user specifies an invalid value:

C<Malformed input-Parameter MyParameter must match pattern
[A-Za-z0-9]+>
...
C<Malformed input-Parameter MyParameter must only contain uppercase and

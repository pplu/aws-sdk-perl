...
As an example, if Att1 is expected to be a Paws::Glue::TaskRunSortCriteria object:

  $service_obj->Method(Att1 => { Column => $value, ..., SortDirection => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Glue::TaskRunSortCriteria object:

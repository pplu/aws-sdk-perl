...
As an example, if Att1 is expected to be a Paws::RDS::CharacterSet object:

  $service_obj->Method(Att1 => { CharacterSetDescription => $value, ..., CharacterSetName => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::RDS::CharacterSet object:

...
An attribute name of C<restore> applies to the list of AWS accounts
that have permission to copy or restore the manual DB snapshot.
...
If the C<AttributeName> field is C<restore>, then this field returns a
list of AWS account ids that are authorized to copy or restore the
manual DB snapshot. If a value of C<all> is in the list, then the
manual DB snapshot is public and available for any AWS account to copy
or restore.

...
package Paws::AppMesh::VirtualGatewayTlsValidationContextSdsTrust;
  use Moose;
...
Paws::AppMesh::VirtualGatewayTlsValidationContextSdsTrust
...
As an example, if Att1 is expected to be a Paws::AppMesh::VirtualGatewayTlsValidationContextSdsTrust object:

  $service_obj->Method(Att1 => { SecretName => $value, ..., SecretName => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::AppMesh::VirtualGatewayTlsValidationContextSdsTrust object:
...
The proxy must be configured with a local SDS provider via a Unix
Domain Socket. See App Mesh TLS documentation

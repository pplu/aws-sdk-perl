# Generated by default/object.tt
package Paws::Macie2::AccountLevelPermissions;
  use Moose;
  has BlockPublicAccess => (is => 'ro', isa => 'Paws::Macie2::BlockPublicAccess', request_name => 'blockPublicAccess', traits => ['NameInRequest']);

1;

### main pod documentation begin ###

=head1 NAME

Paws::Macie2::AccountLevelPermissions

=head1 USAGE

This class represents one of two things:

=head3 Arguments in a call to a service

Use the attributes of this class as arguments to methods. You shouldn't make instances of this class. 
Each attribute should be used as a named argument in the calls that expect this type of object.

As an example, if Att1 is expected to be a Paws::Macie2::AccountLevelPermissions object:

  $service_obj->Method(Att1 => { BlockPublicAccess => $value, ..., BlockPublicAccess => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Macie2::AccountLevelPermissions object:

  $result = $service_obj->Method(...);
  $result->Att1->BlockPublicAccess

=head1 DESCRIPTION

Provides information about the account-level permissions settings that
apply to an S3 bucket.

=head1 ATTRIBUTES


=head2 BlockPublicAccess => L<Paws::Macie2::BlockPublicAccess>

The block public access settings for the AWS account that owns the
bucket.



=head1 SEE ALSO

This class forms part of L<Paws>, describing an object used in L<Paws::Macie2>

=head1 BUGS and CONTRIBUTIONS

The source code is located here: L<https://github.com/pplu/aws-sdk-perl>

Please report bugs to: L<https://github.com/pplu/aws-sdk-perl/issues>

=cut


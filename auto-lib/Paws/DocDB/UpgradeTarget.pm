package Paws::DocDB::UpgradeTarget;
  use Moose;
...
As an example, if Att1 is expected to be a Paws::DocDB::UpgradeTarget object:

  $service_obj->Method(Att1 => { AutoUpgrade => $value, ..., IsMajorVersionUpgrade => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::DocDB::UpgradeTarget object:
...
source DB instances that have C<AutoMinorVersionUpgrade> set to
C<true>.

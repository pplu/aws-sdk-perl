...
Paws::SimpleWorkflow::ContinueAsNewWorkflowExecutionFailedEventAttributes
...
As an example, if Att1 is expected to be a Paws::SimpleWorkflow::ContinueAsNewWorkflowExecutionFailedEventAttributes object:

  $service_obj->Method(Att1 => { Cause => $value, ..., DecisionTaskCompletedEventId => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SimpleWorkflow::ContinueAsNewWorkflowExecutionFailedEventAttributes object:
...
Provides details of the C<ContinueAsNewWorkflowExecutionFailed> event.

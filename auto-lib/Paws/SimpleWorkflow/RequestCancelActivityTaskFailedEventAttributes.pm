...
Paws::SimpleWorkflow::RequestCancelActivityTaskFailedEventAttributes
...
As an example, if Att1 is expected to be a Paws::SimpleWorkflow::RequestCancelActivityTaskFailedEventAttributes object:

  $service_obj->Method(Att1 => { ActivityId => $value, ..., DecisionTaskCompletedEventId => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SimpleWorkflow::RequestCancelActivityTaskFailedEventAttributes object:
...
Provides the details of the C<RequestCancelActivityTaskFailed> event.

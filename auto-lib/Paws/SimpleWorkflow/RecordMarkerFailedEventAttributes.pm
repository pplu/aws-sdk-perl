...
Paws::SimpleWorkflow::RecordMarkerFailedEventAttributes
...
As an example, if Att1 is expected to be a Paws::SimpleWorkflow::RecordMarkerFailedEventAttributes object:

  $service_obj->Method(Att1 => { Cause => $value, ..., MarkerName => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SimpleWorkflow::RecordMarkerFailedEventAttributes object:
...
Provides the details of the C<RecordMarkerFailed> event.
...
decision task that resulted in the C<RecordMarkerFailed> decision for

...
Paws::Greengrass::FunctionExecutionConfig
...
As an example, if Att1 is expected to be a Paws::Greengrass::FunctionExecutionConfig object:

  $service_obj->Method(Att1 => { IsolationMode => $value, ..., RunAs => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Greengrass::FunctionExecutionConfig object:

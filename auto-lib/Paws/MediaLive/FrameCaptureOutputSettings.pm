...
Paws::MediaLive::FrameCaptureOutputSettings
...
As an example, if Att1 is expected to be a Paws::MediaLive::FrameCaptureOutputSettings object:

  $service_obj->Method(Att1 => { NameModifier => $value, ..., NameModifier => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::MediaLive::FrameCaptureOutputSettings object:

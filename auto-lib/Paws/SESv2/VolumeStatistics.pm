...
As an example, if Att1 is expected to be a Paws::SESv2::VolumeStatistics object:

  $service_obj->Method(Att1 => { InboxRawCount => $value, ..., SpamRawCount => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::SESv2::VolumeStatistics object:

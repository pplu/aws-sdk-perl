...
As an example, if Att1 is expected to be a Paws::WorkSpaces::RebootRequest object:

  $service_obj->Method(Att1 => { WorkspaceId => $value, ..., WorkspaceId => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::WorkSpaces::RebootRequest object:

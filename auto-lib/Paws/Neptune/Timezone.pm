...
As an example, if Att1 is expected to be a Paws::Neptune::Timezone object:

  $service_obj->Method(Att1 => { TimezoneName => $value, ..., TimezoneName => $value  });

=head3 Results returned from an API call

Use accessors for each attribute. If Att1 is expected to be an Paws::Neptune::Timezone object:
